  ...
  const handleLenisScroll = useCallback(
    (e: { scroll: number }) => {
      ...
      const maxScrollTop =
        ...
        itemStackDistance * (cards.length - 1);

      if (e.scroll > maxScrollTop) {
        lenisRef.current?.scrollTo(maxScrollTop, { immediate: true });
        return;
      }

      updateCardTransforms();
    },
    [itemStackDistance, stackPosition, updateCardTransforms, parsePercentage]
  );
